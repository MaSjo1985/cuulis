...
<head>
<title> Muokkaa tavoitteiden tasoja </title>';
include("yhteys.php");
...
//        }
        ...
        echo '<form action="muokkaatasoja2.php" class="form-style-spinner"  method="post"><fieldset>';
        $maara = 0;
        echo '<legend>Muokkaa tavoitetasojen rajoja</legend>';
        echo '<a href="itsetyot.php?i=' . $ipid . '" class="erilainen">&#8630 &nbsp&nbsp&nbsp Palaa takaisin</a><br><br><br>';
        while ($row = $onkorivi2->fetch_assoc()) {
            $maara++;
            $row[selite] = str_replace('<br />', "", $row[selite]);
            echo'<p style="width: 30%"><b style="margin-right:10px">Tason kuvaus: &nbsp&nbsp&nbsp</b><textarea style="display: inline-block; overflow-x: hidden; width: 50%" name="selite[]" rows="1" >' . $row[selite] . '</textarea></p>';
            echo'<p style="width: 35%"><b>Osuus maksimipisteistä (%): &nbsp&nbsp&nbsp</b><input type="text" id="spinner' . $maara . '" style="width: 50px" name="osuus[]" value=' . $row[osuus] . '></p>';

            echo'<input type="hidden" name="id[]" value=' . $row[id] . '>';
            echo'<input type="hidden" name="lista" value=' . $row[id] . '>';
            echo'<p style="width: 10%"><button title="Poista taso" name="painikep"><i class="fa fa-trash-o" style="margin-right: 10px;"></i>Poista</button><p><br>';
        }
        echo'<br>';
        echo'<input type="hidden" id="ipid" name="ipid" value=' . $ipid . '>';
        echo'<input type="hidden" id="maara" name="maara" value=' . $maara . '>';
        echo'<p style="display: block"><input type="submit" name="painikel" value="+ Lisää uusi taso" class="myButton8"  role="button"  style="padding:2px 4px; font-size: 0.9em">';

        echo'<br><br><br><input type="submit" name="painiket" value="&#10003 Tallenna" class="myButton9"  role="button"  style="padding:4px 6px; font-size: 0.9em">';
