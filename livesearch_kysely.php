...
if (isset($_POST['search'])) {
 ...
 echo'<br><b style="color: #c7ef00" >Klikkaa sen kurssin/opintojakson nimeä, jonka kyselylomakkeen haluat tuoda.</b><br><br>';
        ...
        while ($stmt->fetch()) {
            ...
            echo '<tr><td><a href="tuokyselylomake3.php?id=' . $row[kid] . '&ipid=' . $ipid . '&monesko=' . $_GET[monesko] . '">' . $row[koodi] . '</a></td><td><a href="tuokyselylomake3.php?id=' . $row[kid] . '&ipid=' . $ipid . '&monesko=' . $_GET[monesko] . '">' . $row[nimi] . '</a></td><td>' . $row[etunimi] . ' ' . $row[sukunimi] . '</td><td>' . $row[Nimi] . '</td><td>' . $row[lukuvuosi] . '</td><td>' . $row[alkupvm] . '</td><td>' . $row[loppupvm] . '</td></tr>';
        }
        echo "</table>";
        echo "</div>";
        echo "<br>";
