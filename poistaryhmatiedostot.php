  ...
  <div class="bar3"></div></a>
    ...
    if ($_POST["valinta"] == "ei") {
        ...
        foreach ($lista as $tuote) {
            ...
            if (!$result = $db->query("select distinct * from ryhmat2 where id = '" . $tuote . "'")) {
            ...
            if ($linkki == 1) {
                $db->query("delete from ryhmat2 where id = '" . $tuote . "'");
                $db->query("delete from opiskelijan_kurssityot where ryhmat2_id = '" . $tuote . "'");
            } else {
                if (file_exists($nimi)) {
                    unlink($nimi);
                }
