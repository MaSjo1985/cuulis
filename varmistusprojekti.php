...
<head>
<title> Palautukset-osion poisto </title>';
...
';
        ...
        if (!$haenimi = $db->query("select * from projektit where id='" . $_POST[id] . "'")) {
            die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="yhteydenotto.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
        }
//projektin id
        while ($row = $haenimi->fetch_assoc()) {
            $nimi = $row[kuvaus];
        }

        echo '<p><b>Haluatko todella poistaa Palautus-osion </b>' . $nimi . '?</p>';


        echo '<br><a href="poistaprojekti.php?id=' . $_POST[id] . '" class="myButton9"  role="button"  style="margin-right: 30px">Kyllä</a>';
        echo '<a href="ryhmatyot.php?r=' . $_POST[id] . '" class="myButton9"  role="button"  style="margin-right: 30px">En</a><br>';
