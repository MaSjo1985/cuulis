...
//TULEE VAIN YKSI TIEDOSTO!!
        ...
        try {
            ...
            $stmt = $db->prepare("INSERT INTO ryhmat2 (ryhma_id, projekti_id, tyonimi, tallennettunimi, omatallennusnimi) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("iisss", $ryhma, $projekti, $tyonimi, $tallennettunimi, $omatallennusnimi);
            $paateloyty = false;
...
//echo'<br>Pääte on: '.$paate;
//   echo'<br>substr on: '. $nimi[$j];
...
//    $nimi2 = preg_replace("/[^A-Z0-9._-]/i", "_",  $nimi[$j]);
            ...
            if (!file_exists($kohde)) {
                ...
                if (!@move_uploaded_file($_FILES['my_file']['tmp_name'], $kohde)) {
            ...
            die('<b style="font-size: 1em; color: #FF0000">' . $e->getMessage() . '</b><br><br><a href="tiedosto.php?pid=' . $_POST[pid] . '"><p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin</a><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
        }
    }
