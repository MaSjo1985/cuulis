...
function myFunction(y) {
  y.classList.toggle("change");
    ...
    if (empty($_GET["lista"])) {
        echo '<p id="ohje"><b style="font-size: 1.1em">Et valinnut yhtään tiedostoa!</b></p>';
        echo'<br><a href="tuoopetiedosto2.php?id=' . $_GET[id] . '&kid=' . $_GET[kid] . '"> <p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin';
    } else {
        $lista = $_GET["lista"];
        ...
        foreach ($lista as $tuote) {
            if (!$haetiedosto = $db->query("select distinct tiedostot.vanhalinkki as vanhalinkki, tiedostot.upotus as upotus, tiedostot.youtube as youtube, tiedostot.linkki as linkki, tiedostot.nimi as nimi, tiedostot.omatallennusnimi as omatallennusnimi, tiedostot.kuvaus as kuvaus from kansiot, tiedostot where kansiot.id='" . $_GET[kid] . "' AND tiedostot.id='" . $tuote . "'")) {
                die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="yhteydenotto.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
            }
            ...
            while ($rowt = $haetiedosto->fetch_assoc()) {
                ...
                if ($rowt[linkki] == 0) {
                    ...
                    $db->query("insert into tiedostot (kansio_id, kuvaus, omatallennusnimi, nimi, tuotu, linkki, vanhalinkki, upotus, youtube) values('" . $_GET[kid] . "', '" . $rowt[kuvaus] . "', '" . $rowt[omatallennusnimi] . "', '" . $rowt[nimi] . "', 1, 0, '" . $rowt[vanhalinkki] . "', '" . $rowt[upotus] . "', '" . $rowt[youtube] . "')");
                } else {

                    $db->query("insert into tiedostot (kansio_id, kuvaus, omatallennusnimi, nimi, tuotu, linkki, vanhalinkki, upotus, youtube) values('" . $_GET[kid] . "', '" . $rowt[kuvaus] . "', '" . $rowt[omatallennusnimi] . "', '" . $rowt[nimi] . "', 1, 1, '" . $rowt[vanhalinkki] . "', '" . $rowt[upotus] . "', '" . $rowt[youtube] . "')");
                }
            }
        }
        header("location: tiedostot.php?k=" . $_GET[kid]);
    }
