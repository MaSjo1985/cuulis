...
<div class="cm8-half" style="padding-top: 0px; margin-top: 0px">';
    ...
    echo'<br><br>Tehtävää <b>' . $sisalto . '</b> on kommentoitu seuraavasti: <br>';
    echo'<div class="cm8-responsive cm8-margin-top">';
    echo '<table id="mytable" class="cm8-table cm8-bordered"><thead>';
    echo '<tr style="background-color: #ffcceb"><th>Opiskelija</th><th style="padding-top: 10px">Kommentti</th></tr></thead><tbody>';


    if (!$haekommentit = $db->query("select distinct * from itsetehtavatkp where itsetehtavat_id='" . $_GET[tid] . "' AND kommentti<>''")) {
        die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
    }
    ...
    while ($rowkommentti = $haekommentit->fetch_assoc()) {
        ...
        if (!$haeopiskelijat = $db->query("select distinct * from kayttajat where id='" . $rowkommentti["kayttaja_id"] . "' ORDER BY sukunimi asc, etunimi")) {
            die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
        }
        while ($rowop = $haeopiskelijat->fetch_assoc()) {

            echo '<tr><td>' . $rowop[sukunimi] . " " . $rowop[etunimi] . '</td><td>' . $rowkommentti[kommentti] . '</td></tr>';
        }
    }
