...
function myFunction(y) {
 ...
 echo'<button  name="painike" title="Tuo Palautus-osio" class="myButton8" style="font-size: 0.8em"><i class="fa fa-recycle"></i>&nbsp&nbsp Tuo Palautus-osio </button>';
  ...
  echo'</nav>
    ...
    if ($linkki == 1) {
        $db->query("delete from ryhmat2 where id = '" . $_GET[id] . "'");
        $db->query("delete from opiskelijan_kurssityot where ryhmat2_id = '" . $_GET[id] . "'");
        header("location: ryhmatyot.php?r=" . $pid . "#" . $_GET[ryid]);
    } else {
        if (file_exists($nimi)) {
            unlink($nimi);
        }
        ...
        if (file_exists($nimi)) {
            ...
            echo'<b style="color: #e608b8"><br>Tiedostoa ' . $omanimi . ' ei pystytty poistamaan!</b> <br><br><b style="color: #e608b8">Voit ottaa yhteyttä oppimisympäristön ylläpitäjään <a href="yhteydenotto.php" style="text-decoration: underline"><u>tästä.</b></u></b><br><br><br><a href="ryhmatyot.php?r=' . $_GET[pid].'#'.$_GET[ryid].'"><p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin</a>';
