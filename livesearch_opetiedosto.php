...
if (isset($_POST['search'])) {
 ...
 echo'<br><b style="color: #e608b8" >Klikkaa sen kurssin/opintojakson nimeä, josta haluat tuoda tiedostoja/linkkejä.</b><br><br>';
       ...
       while ($stmt->fetch()) {
            ...
            $row[loppupvm] = date("d.m.Y", strtotime($row[loppupvm]));
                ...
                while ($rowope = $resulthaeope->fetch_assoc()) {
                 ...
                 echo '<tr><td><a href="tuoopetiedosto2.php?id=' . $row[kid] . '&kid=' . $_POST[kid] . '&monesko=' . $_GET[monesko] . '">' . $row[koodi] . '</a></td><td><a href="tuoopetiedosto2.php?id=' . $row[kid] . '&kid=' . $_POST[kid] . '&monesko=' . $_GET[monesko] . '">' . $row[nimi] . '</a></td><td>' . $etunimi . ' ' . $sukunimi . '</td><td>' . $row[Nimi] . '</td><td>' . $row[lukuvuosi] . '</td><td>' . $row[alkupvm] . '</td><td>' . $row[loppupvm] . '</td></tr>';
