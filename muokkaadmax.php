...
<a href="itsetyot.php?i=' . $eka_id . '"';
 ...
 <div class="cm8-margin-top"></div>';
    ...
    if ($_SESSION["Rooli"] == "opettaja" || $_SESSION["Rooli"] == "admin" || $_SESSION["Rooli"] == "admink" || $_SESSION["Rooli"] == "opeadmin") {
        ...
        if (!$onkorivi2 = $db->query("select distinct dmax from itseprojektit where id='" . $ipid . "'")) {
            die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
        }
        ...
        echo '<form action="muokkaadmax2.php"  method="post" class="form-style-k"><fieldset>';

        echo '<legend>Muokkaa tehtävädiagrammin maksimiprosenttimäärää</legend>';
        echo '<a href="itsetyot.php?i=' . $ipid . '" class="palaa">&#8630 &nbsp&nbsp&nbspPalaa takaisin</a><br><br>';
        while ($row = $onkorivi2->fetch_assoc()) {

            echo'<br><p style="display: inline-block;"><b style="margin-right: 20px">Tehtävädiagrammin maksimiprosenttimäärä: </b><input type="text" id="spinner" style="width: 60px" name="minimi" value=' . $row[dmax] . ' > <b>%</b></p>';
        }
