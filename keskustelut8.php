...
function myFunction(y) {
  y.classList.toggle("change");
    ...
    echo'<div class="cm8-third" style="padding-left: 20px; width: 25% "><h2 style="padding-top: 0px; padding-left: 0px; padding-bottom: 0px" id="lisaa">Keskustele</h2>';
    echo'<div class="cm8-margin-top" ></div>';
    ...
    if ($haeprojekti->num_rows != 0) {
        ...
        while ($rowP = $haeprojekti->fetch_assoc()) {
            ...
            if ($_GET[r] == $id || $_GET[r] == $idtoinen) {
                ...
                echo'<a href="keskustelut.php?r=' . $id . '" class="btn-info3" style="margin-right: 20px; margin-bottom: 5px;  padding: 3px 6px 3px 6px"><b style="font-size: 1em; ">&#9997 &nbsp&nbsp&nbsp' . $otsikko . ' </b></a>';
            }
        }
    }
    echo'</nav>';
    ...
    echo'<div class="cm8-third" style="padding-top: 30px; width: 50%">';
...
<br><br><input type="submit" onClick="submitChat2()"  class="myButton9" value="&#10147 Lähetä" /> 
        ...
        if ($akt == 0) {
            echo'<br><em>Ei keskusteluja.</em><form action="aktivoikeskustelu.php" method="post"><br><br><input type="hidden" name="id" value=' . $_SESSION["KurssiId"] . '><input type="submit" name="painikea" value="+ Lisää keskustelu" class="myButton8" role="button" style="font-size: 0.8em; padding:2px 4px"></form></div>';
        } else if ($akt == 1 && !isset($_GET[r]) && !$onyksi) {
            echo'Valitse haluamasi keskustelu';
        } else if ($akt == 1 && !isset($_GET[r]) && $onyksi) {
            if (!$hae_eka = $db->query("select * from kurssin_keskustelut where kurssi_id='" . $_SESSION["KurssiId"] . "'")) {
            ...
            if ($haeakt->num_rows != 0) {
                ...
                echo'<b>Nimimerkki:</b> <br><br><textarea name="nimi" rows="2" style="font-size:0.9em">' . $nimi . '</textarea><br><br><b>Viesti:</b> <br> <br><textarea id="sendie"  class="content" name="uusi" rows="2" style="font-size:0.9em" ></textarea>
								<input type="hidden" name="paiva" value=' . $paiva . '>
								<input type="hidden" name="kello" value=' . $kello . '>
                                                              <input type="hidden" name="id" value=' . $_GET[r] . '>
                                                                   <input type="hidden" id="haeid" value=' . $_GET[r] . '>
