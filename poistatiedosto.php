...
function myFunction(y) {
 ...
 <div class="cm8-margin-top"></div></div>';
    ...
    if ($linkki == 1) {
        ...
        if (file_exists($nimi)) {
            echo'<br>Tiedostoa ei pystytty poistamaan! <br><br><a href="ryhmatyot.php?r=' . $_GET[pid] . '#' . $_GET[ryid] . '"><p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin</a><br><br>Voit ottaa yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br><br>';
        } else {
            $db->query("delete from ryhmat2 where id = '" . $_GET[id] . "'");
            $db->query("delete from opiskelijan_kurssityot where ryhmat2_id = '" . $_GET[id] . "'");
            header("location: ryhmatyot.php?r=" . $pid . "#" . $_GET[ryid]);
