...
function myFunction(y) {
  y.classList.toggle("change");
    ...
    $headers .= "From: Cuulis-oppimisymparisto <no-reply@cuulis.fi>" . "\r\n";

    $otsikko = "Ryhmävahvistus Cuulis-oppimisympäristöstä";
    ...
    if (!$result = $db->query("select distinct sposti, rooli, opiskelija_id from kayttajat, opiskelijankurssit where kayttajat.id=opiskelijankurssit.opiskelija_id AND opiskelijankurssit.projekti_id='" . $_GET[pid] . "'")) {
        die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
    }

    while ($row = $result->fetch_assoc()) {
        $sposti = $row[sposti];
        $opid = $row[opiskelija_id];

        if (!$result2 = $db->query("select distinct nimi from ryhmat, opiskelijankurssit where ryhmat.id=opiskelijankurssit.ryhma_id AND opiskelijankurssit.opiskelija_id='" . $opid . "'")) {
            die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
        }
...
//            $varmistus = mail($sposti, $otsikko, $viesti, $headers);
    ...
    if ($viesti) {
        header("location: suljekasa2.php");
    } else {
        echo "<br>Ryhmäilmoittautuminen on suljettu, mutta viestin lähettäminen opiskelijoille ei onnistunut.";
        echo '<br><br><a href="ryhmatyot.php"><p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin projektisivulle</a>';
    }
} else {
