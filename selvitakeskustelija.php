  ...
  y.classList.toggle("change");
    ...
    if (x.className === "topnav") {
        ...
        if ($akt == 0) {
            ...
            if (!$haeprojekti = $db->query("select * from kurssin_keskustelut where id='" . $_GET[id] . "'")) {
                die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="yhteydenotto.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
            }
            ...
            if ($haeprojekti->num_rows != 0) {
                ...
                while ($rowP = $haeprojekti->fetch_assoc()) {
                    ...
                    $id = $rowP[id];
                    $idtoinen = $rowP[id] . "/";
                    if ($_GET[id] == $id || $_GET[id] == $idtoinen) {

                        echo'<a href="keskustelut.php?r=' . $id . '" class="btn-info3-valittu" style="margin-right: 20px; margin-bottom: 5px;  padding: 3px 6px 3px 6px"><b style="font-size: 1em; ">&#9997 &nbsp&nbsp&nbsp' . $otsikko . ' </b></a>';
                    }
                }
            }
            ...
            if ($haeprojekti->num_rows != 0) {
                ...
                while ($rowP = $haeprojekti->fetch_assoc()) {
                    ...
                    if ($_GET[id] != $id && $_GET[id] != $idtoinen)
                        echo'<a href="keskustelut.php?r=' . $id . '" class="btn-info3" style="margin-right: 20px; margin-bottom: 5px; padding: 6px 6px 6px 6px"><b style="font-size: 1em; ">' . $otsikko . '</b></a>';
                }
        ...
        if (!$haekeskustelu = $db->query("select distinct * from keskustelut where id='" . $_GET["kesid"] . "'")) {
            die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="yhteydenotto.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
        }

        while ($rowvi = $haekeskustelu->fetch_assoc()) {
            $sisalto = $rowvi[sisalto];
        }

        if (isset($_GET[valitse])) {
            echo'<br><a href="valitsekeskustelu.php?id=' . $_GET[id] . '"><p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin </a><br><br>';
        } else {
            echo'<br><a href="keskustelut.php?r=' . $_GET[id] . '"><p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin </a><br><br>';
        }

        echo'<br><br>Viestin &nbsp&nbsp <b>"' . $sisalto . '"</b> &nbsp&nbsp lähetti &nbsp&nbsp ' . $nimi . '
    
		<br><form action="poistakeskusteluviestiyksilollinen.php" method="get">';
        echo'<br><input type="hidden" name="kesid" value=' . $_GET[kesid] . '><input type="hidden" name="id" value=' . $_GET[id] . '><button class="pieniroskis" title="Poista viesti"><i class="fa fa-trash-o" style="margin-right: 10px;"></i>Poista</button>';
        if (isset($_GET[valitse])) {
            echo '<input type="hidden" name="valitse" value=1>';
        }
