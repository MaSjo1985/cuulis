...
function myFunction(y) {
  y.classList.toggle("change");
    ...
    echo'<div class="cm8-threequarter" style="margin-left: 40px;padding-top: 0px; margin-top: 0px; margin-bottom: 0px; padding-bottom: 10px">';
    ...
    if (!$result = $db->query("select distinct * from ryhmat2 where id = '" . $_POST[id] . "'")) {
        die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="yhteydenotto.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
    }
    ...
    if ($linkki == 1) {
        echo '<br><p><b>Haluatko todella poistaa tiedoston</b> "' . $tyonimi . '"?</p>';
    } else {
        echo '<br><p><b>Haluatko todella poistaa tiedoston </b>"' . $omatallennusnimi . '"?</p>';
    }


    echo '<a href="poistatiedosto.php?id=' . $id . '&pid=' . $_POST[pid] . '&ryid=' . $_POST[ryid] . '" class="myButton9"  role="button"  style="margin-right: 30px">Kyllä</a>';
    echo '<a href="ryhmatyot.php?r=' . $pid . '#' . $_POST[ryid] . '" class="myButton9"  role="button"  style="margin-right: 30px">En</a><br>';
} else {
