...
if (isset($_POST['search'])) {
    ...
    $url = "kayttajatopettajat.php";
    $hakusanao = mysqli_real_escape_string($db, $_POST['search']);
    $hakusanao = trim($hakusanao);
    ...
    if (isset($_POST['sorting3'])) {
        ...
        if ($_POST['sorting3'] == 'ASC') {
            $sort3 = 'DESC';
        } else {
            $sort3 = 'ASC';
        }
    }
    if ($_POST['field3'] == 'sukunimi') {
        $field3 = "sukunimi";
    } elseif ($_POST['field3'] == 'etunimi') {
        $field3 = "etunimi";
    } elseif ($_POST['field3'] == 'sposti') {
        $field3 = "sposti";
    } elseif ($_POST['field3'] == 'Nimi') {
        $field3 = "Nimi";
    }
    ...
    if ($_SESSION["Rooli"] == 'admin') {
        ...
        $stmt = $db->prepare("SELECT DISTINCT etunimi, sukunimi, sposti, Nimi, kayttajat.id as kaid, paiva, kello FROM kayttajat, kayttajankoulut, koulut WHERE kayttajankoulut.odottaa=1 AND  kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND rooli='opettaja' AND kayttajat.tarkistettu=1 AND kayttajat.vahvistettu=1 AND (sposti like ? OR etunimi like ? OR sukunimi like ? OR Nimi like ? OR kokonimi like ?) ORDER BY $field3 $sort3");
        ...
        $stmt = $db->prepare("SELECT DISTINCT etunimi, sukunimi, sposti, Nimi, kayttajat.id as kaid, paiva, kello FROM kayttajat, kayttajankoulut, koulut WHERE koulut.id='" . $_SESSION["kouluId"] . "' AND kayttajankoulut.odottaa=1 AND  kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND rooli='opettaja' AND kayttajat.tarkistettu=1 AND kayttajat.vahvistettu=1 AND (sposti like ? OR etunimi like ? OR sukunimi like ? OR Nimi like ? OR kokonimi like ?) ORDER BY $field3 $sort3");
        $stmt->bind_param("sssss", $s1, $s1, $s1, $s1, $s1);
        // prepare and bind
        $s1 = "%" . $hakusanao . "%";
        $stmt->execute();
