...
';
            ...
            echo' <a href="keskustelut.php" >Keskustele</a> 
	  <a href="osallistujat.php"   >Osallistujat</a>  	  
	   <a href="javascript:void(0);" class="icon" onclick="myFunction(this)"><div class="bar1"></div>
  <div class="bar2"></div>
  <div class="bar3"></div></a>
...
<div class="cm8-half" style="padding-top: 0px">';
        ...
        if (empty($_POST["lista"])) {
            echo '<p style="font-weight: bold" >Et valinnut yhtään kysymystä!</p>';

            echo'<br><br><a href="valitsekysymykset.php?w2=' . $_POST[w2] . '&w1=' . $_POST[w1] . '"> <p style="font-size: 1em; display: inline-block; padding:0; margin: 0px 20px 0px 0px">&#8630</p> Palaa takaisin';
        } else {
            $lista = $_POST["lista"];
            $idt = $_POST["lista"];
            echo '<p style="font-weight: bold" >Haluatko todella poistaa valitut kysymykset?</p>';


            foreach ($lista as $tuote) {

                if (!$result = $db->query("select distinct * from kysymykset where id = '" . $tuote . "'")) {
                    die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
                }
            ...
            echo'<form action="poistakysymys.php" method="post">
		   ...
		   <input type="radio" name = "valinta" value="ei" style="font-size: 1em" selected> En <br>';
            ...
            echo'<input type="hidden" name="w2" value=' . $_POST[w2] . '>
			<input type="hidden" name="w1" value=' . $_POST[w1] . '>';
