...
function myFunction(y) {
 ...
 <div class="cm8-margin-top"></div></div>';
    ...
    if (!$result = $db->query("select distinct * from ryhmatope where id = '" . $_POST[id] . "'")) {
        die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
    }
    ...
    echo '<a href="poistatiedosto_ope.php?id=' . $id . '&pid=' . $_POST[pid] . '&ryid=' . $_POST[ryid] . '" class="myButton9"  role="button"  style="margin-right: 30px">Kyllä</a>';
    echo '<a href="ryhmatyot.php?r=' . $_POST[pid] . '#' . $_POST[ryid] . '"  class="myButton9"  role="button"  style="margin-right: 30px">En</a><br>';
} else {
