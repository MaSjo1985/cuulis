...
if (isset($_SESSION["Kayttajatunnus"])) {
    ...
    if ($_POST["valinta"] == "ei")
        ...
        if (!$tulos41 = $db->query("select distinct * from kurssit where id='" . $_SESSION["KurssiId"] . "'")) {
            die('Tietokantahaussa ilmeni ongelmia [' . $db->error . ']');
        }

        while ($rivi21 = $tulos41->fetch_assoc()) {
            $Nimi = $rivi21[nimi];
        }
        ...
        foreach ($lista as $tuote) {
            ...
            $viesti = 'Sinut on poistettu kurssilta/opintojaksolta' . $Nimi . ' Cuulis-oppimisympäristössä.<br><br><em>Tähän viestiin ei voi vastata.</em>';
            $viesti = str_replace("\n.", "\n..", $viesti);
