  ...
  y.classList.toggle("change");
    ...
    CAST(SUBSTR(nimi FROM 3) AS UNSIGNED)")) {
        ...
        if ($_SESSION["Rooli"] == 'opettaja' || $_SESSION["Rooli"] == 'admin' || $_SESSION["Rooli"] == 'admink' || $_SESSION["Rooli"] == 'opeadmin') {
            ...
            echo'<div id="content" class="cm8-twothird" style="padding-left: 20px; margin-right: 0px; margin-top: 20px; margin-bottom: 0px; padding-bottom: 10px">';

            echo '<form action="lisaakansio.php" method="post" class="form-style-k" style="width: 50%" ><fieldset>';
            echo' <legend>Lisää uusi kansio</legend>';
            echo' <a href="tiedostot.php" class="palaa">&#8630 &nbsp&nbsp&nbspPalaa takaisin</a><br><br><br>';


            echo'<p>Anna kansiolle nimi: <br><input type="text" id="nimi" name="nimi" maxlength="60"></p>

	
	<input type="hidden" name="kurssiId" value=' . $_SESSION["KurssiId"] . '>  						
	<br><br><input id="button" type="submit" value="&#10003 Lisää kansio" class="myButton9">
	</fieldset></form>';
        } else {
