...
if (isset($_POST['search'])) {
    ...
    if ($stmt->num_rows == 0)
        ...
        if ($_SESSION["Rooli"] == 'admin') {
            ...
            echo '<tr><th>Sukunimi</th><th>Etunimi</th><th>Rooli</th><th>Sähköpostiosoite</th><th>Oppilaitos</th><th>Kirjautunut viimeksi</th><th>Valitse<br>&nbsp&#9661&nbsp</th><th>Lähetä viesti</th></tr></thead><tbody>';
            ...
            while ($stmt->fetch()) {
                ...
                echo '<tr><td><a href="kayttaja.php?url=' . $url . '&ka=' . $row10[kaid] . '">' . $row10[sukunimi] . '</a></td><td><a href="kayttaja.php?url=' . $url . '&ka=' . $row10[kaid] . '">' . $row10[etunimi] . "</a></td><td>" . $row10[rooli] . '</td><td>' . $row10[sposti] . '</td><td>' . $row10[Nimi] . '</td><td>' . $kirjautunut . '</td><td style="padding-left: 10px"><input type="checkbox" name="lista10[]" value=' . $row10[kaid] . ' ></td><td><a href="viestikayttajalle.php?url=' . $url . '&id=' . $row10[kaid] . '" style="padding: 0px 4px; margin: 0" title="Lähetä viesti käyttäjälle">📧 &nbsp</a></td></tr>';
            }
