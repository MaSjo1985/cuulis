...
$start_from = ($page-1) * $results_per_page;
        ...
        if ($_SESSION["Rooli"] == 'admin') {
            ...
            if ($_GET['field10'] == 'kirjautunut') {
                ...
                if (!$result10 = $db->query("select distinct paiva, kello, sukunimi, etunimi, sposti, Nimi, kayttajat.id as kaid from kayttajat, koulut, kayttajankoulut where kayttajankoulut.odottaa=1 AND kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND kayttajat.tarkistettu=1 AND  rooli='opettaja' ORDER BY paiva $sort, kello LIMIT $start_from, $results_per_page")) {
                    die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
                }
            } else {
...
for ($i=1; $i<=$total_pages; $i++) {  // print links for all pages
  ...
  echo'</div>';
            ...
            if ($_GET['field10'] == 'kirjautunut') {
                ...
                if (!$result10 = $db->query("select distinct paiva, kello, sukunimi, etunimi, sposti, Nimi, kayttajat.id as kaid from kayttajat, koulut, kayttajankoulut where koulut.id='" . $_SESSION["kouluId"] . "' AND kayttajankoulut.odottaa=1 AND kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND kayttajat.tarkistettu=1 AND  rooli='opettaja' ORDER BY paiva $sort, kello LIMIT $start_from, $results_per_page")) {
                    die('<br><br><b style="font-size: 1em; color: #FF0000">Tietokantayhteydessä ongelmia!<br><br> Ota yhteyttä oppimisympäristön ylläpitäjään <a href="bugi.php" style="text-decoration: underline"><u>tästä.</b></u><br><br></div></div></div></div><footer class="cm8-containerFooter" style="padding: 20px 0px 20px 0px"><b>Copyright &copy;  <br><a href="admininfo.php">Marianne Sjöberg</b></a></footer>');
                }
            } else {
                if (!$result10 = $db->query("select distinct paiva, kello, sukunimi, etunimi, sposti, Nimi, kayttajat.id as kaid from kayttajat, koulut, kayttajankoulut where koulut.id='" . $_SESSION["kouluId"] . "' AND kayttajankoulut.odottaa=1 AND kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND kayttajat.tarkistettu=1 AND  rooli='opettaja' ORDER BY $field10 $sort LIMIT $start_from, $results_per_page")) {
