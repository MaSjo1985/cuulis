...
if (isset($_POST['search'])) {
    ...
    $sort4 = 'DESC';
    $nuoli4 = "&#8661";
    if (isset($_POST['sorting4'])) {

        if ($_POST['sorting4'] == 'ASC') {
            $sort4 = 'DESC';
        } else {
            $sort4 = 'ASC';
        }
    }
    if ($_POST['field4'] == 'sukunimi') {
        $field4 = "sukunimi";
    } elseif ($_POST['field4'] == 'etunimi') {
        $field4 = "etunimi";
    } elseif ($_POST['field4'] == 'sposti') {
        $field4 = "sposti";
    } elseif ($_POST['field4'] == 'Nimi') {
        $field4 = "Nimi";
    }
    ...
    if ($_SESSION["Rooli"] == 'admin') {
        $stmt = $db->prepare("SELECT DISTINCT etunimi, sukunimi, sposti, Nimi, kayttajat.id as kaid, paiva, kello FROM kayttajat, kayttajankoulut, koulut WHERE kayttajankoulut.odottaa=1 AND  kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND rooli='opiskelija' AND kayttajat.tarkistettu=1 AND kayttajat.vahvistettu=1 AND (sposti like ? OR etunimi like ? OR sukunimi like ? OR Nimi like ? OR kokonimi like ?) ORDER BY $field4 $sort4");
        ...
        $stmt = $db->prepare("SELECT DISTINCT etunimi, sukunimi, sposti, Nimi, kayttajat.id as kaid, paiva, kello FROM kayttajat, kayttajankoulut, koulut WHERE koulut.id='" . $_SESSION["kouluId"] . "' AND kayttajankoulut.odottaa=1 AND  kayttajat.id=kayttajankoulut.kayttaja_id AND koulut.id=kayttajankoulut.koulu_id AND rooli='opiskelija' AND kayttajat.tarkistettu=1 AND kayttajat.vahvistettu=1 AND (sposti like ? OR etunimi like ? OR sukunimi like ? OR Nimi like ? OR kokonimi like ?) ORDER BY $field4 $sort4");
        $stmt->bind_param("sssss", $s1, $s1, $s1, $s1, $s1);
        // prepare and bind
        $s1 = "%" . $hakusanaop . "%";
        $stmt->execute();
    ...
    if ($stmt->num_rows == 0)
        ...
        if ($_SESSION["Rooli"] == 'admin') {
            ...
            echo '<tr><th>Sukunimi</th><th>Etunimi</th><th>Käyttäjätunnus</th><th>Oppilaitos</th><th>Kirjautunut viimeksi</th><th>Valitse<br>&nbsp&#9661&nbsp</th></tr></thead><tbody>';
